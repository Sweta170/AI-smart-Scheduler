...
router.post('/meetings/create', async (req, res) => {
  try {
    ...
    // Add the creator (hostUser) to the attendees list so it shows on their dashboard
    const hostUser = req.user || await User.findOne({ email: "you@company.com" });
    if (hostUser) {
      const hostIdStr = hostUser._id.toString();
      const isHostIncluded = resolvedAttendees.some(id => id.toString() === hostIdStr);
      if (!isHostIncluded) {
        resolvedAttendees.push(hostUser._id);
      }
    }
...
router.get('/meetings/today', async (req, res) => {
  try {
    ...
    const todayDate = new Date(); // Use real current date
...
router.get('/meetings/week', async (req, res) => {
  try {
    ...
    const startOfWeek = new Date(new Date().setDate(now.getDate() - day + 1));
    startOfWeek.setHours(0,0,0,0);
    const endOfWeek = new Date(new Date().setDate(now.getDate() - day + 7));
